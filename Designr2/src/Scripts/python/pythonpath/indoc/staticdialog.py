...
def createDialog(enhancedmouseevent, xscriptcontext, dialogtitle, defaultrows=None, outputcolumn=None, *, callback=None):  # dialogtitleはダイアログのデータ保存名に使うのでユニークでないといけない。defaultrowsはグリッドコントロールのデフォルトデータ。
	...
	gridselectionlistener = GridSelectionListener(args)
	gridcontrol1 = addControl("Grid", gridprops, {"addMouseListener": mouselistener, "addSelectionListener": gridselectionlistener})  # グリッドコントロールの取得。
	...
	gridselectionlistener.dialogframe = dialogframe
	...
	args = doc, actionlistener, dialogwindow, windowlistener, mouselistener, menulistener, controlcontainerwindowlistener, optioncontrolcontainerwindowlistener, gridselectionlistener
...
class CloseListener(unohelper.Base, XCloseListener):  # ノンモダルダイアログのリスナー削除用。
	...
	def queryClosing(self, eventobject, getsownership):  # ノンモダルダイアログを閉じる時に発火。
		...
		doc, actionlistener, dialogwindow, windowlistener, mouselistener, menulistener, controlcontainerwindowlistener, optioncontrolcontainerwindowlistener, gridselectionlistener = self.args
...
class MouseListener(unohelper.Base, XMouseListener):  
	def __init__(self, args): 	
		self.args = args
		self.gridpopupmenu = args[0]  # gridpopupmenuはCloseListenerでも使うので、別でも取得する。
		self.optioncontrolcontainer = None
		self.dialogframe = None
	def mousePressed(self, mouseevent):  # グリッドコントロールをクリックした時。コントロールモデルにはNameプロパティはない。コントロール上のポップアップメニューをクリックしても発火する。
...
# 		optioncontrolcontainer = self.optioncontrolcontainer
		if mouseevent.Buttons==MouseButton.LEFT:
			if mouseevent.ClickCount==1:  # シングルクリックの時。
				return  # GridSelectionListenerで処理するので何もしない。
...
# 				self._toCell(gridcontrol)
				...
				toCell(*self.args, self.dialogframe, self.optioncontrolcontainer, gridcontrol)
...
def toCell(gridpopupmenu, xscriptcontext, outputcolumn, callback, dialogframe, optioncontrolcontainer, gridcontrol):
...
class GridSelectionListener(unohelper.Base, XGridSelectionListener):
	def __init__(self, args): 	
		self.args = args
		self.gridpopupmenu = args[0]  # gridpopupmenuは、別でも取得する。
		self.optioncontrolcontainer = None
		self.optioncontrolcontainer = None
		self.dialogframe = None	
